...
mkdir -p data
mkdir -p data/$MAT_TYPE


find $DATA_FOLDER -name \*$MIC.phn -exec bash -c \
'echo {};cat {}' \; | sed 's/\.phn//' | \
awk '{filename = match($0, "/"); if (filename>0){gsub("/"," ",$0); if (NR!=1) {printf "\n"} {printf "%s%s%s-%s ",$(NF-3),$(NF-2),$(NF-1),$(NF)} } \
else {gsub("@bg","sil",$0);printf "%s ",$3} }' | sort -k1,1 > data/$MAT_TYPE/text


find $DATA_FOLDER -name \*$MIC.wav | sed 's/\.wav//' | \
