...
#  build-lm.sh -i $srcdir/lm_train.text -n 1 -o $tmpdir/lm_phone_bg.ilm.gz

# pure phone loop

  cat $srcdir/train.text | awk '{$1="";print $0}' | tr " " "\n" | sort -n | uniq | awk '{printf "<s> %s </s>\n",$0}' | grep -v '<s> </s>' > $srcdir/lm_train.text

  build-lm.sh -i $srcdir/lm_train.text -n 1 -o $tmpdir/lm_phone_bg.ilm.gz   # .....to select n-gramms
